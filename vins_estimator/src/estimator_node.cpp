...
public:
    ...
    {
        _nh = ros::NodeHandle("~");
        // _nh.setParam("config_file", "/home/rosdev/workspace/ros_ws/src/VINS-Mono/config/euroc/euroc_config.yaml");
    }
    ...
    void Init()
    {
        ...
        _sub_imu = _nh.subscribe("/imu0", 2000, &EstimatorNode::ImuCallback, this, ros::TransportHints().tcpNoDelay());
        _sub_image = _nh.subscribe("/feature_tracker/feature", 2000, &EstimatorNode::FeatureCallback, this);
        _sub_restart = _nh.subscribe("/feature_tracker/restart", 2000, &EstimatorNode::RestartCallback, this);
        _sub_relo_points = _nh.subscribe("/pose_graph/match_points", 2000, &EstimatorNode::RelocalizationCallback, this);
...
private:
    ...
    void Update()
    {
        ...
        estimator_.GetLastestEstiamtedStates(position_estimated_current, orientation_estimated_current, linear_velocity_estimated_current, imu_linear_acceleration_estimated_bias, imu_angular_velocity_estimated_bias);
    ...
    GetMeasurements()
    {
        ...
        while (true)
        {
            ...
            if (!(imu_buf.back()->header.stamp.toSec() > feature_buf.front()->header.stamp.toSec() + estimator_.imu_camera_clock_offset_))
            {
            ...
            if (!(imu_buf.front()->header.stamp.toSec() < feature_buf.front()->header.stamp.toSec() + estimator_.imu_camera_clock_offset_))
            {
            ...
            while (imu_buf.front()->header.stamp.toSec() < img_msg->header.stamp.toSec() + estimator_.imu_camera_clock_offset_)
            {
    ...
    void Process()
    {
        while (true)
        {
            ...
            for (auto &measurement : measurements)
            {
                ...
                {
                    double t = imu_msg->header.stamp.toSec();
                    double img_t = img_msg->header.stamp.toSec() + estimator_.imu_camera_clock_offset_;
                    if (t <= img_t)
                    {
                        if (current_time < 0)
                            current_time = t;
                ...
                header.frame_id = "world";
                estimator_publisher->PublishAll(estimator_, header, whole_t);
