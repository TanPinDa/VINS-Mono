...
void solveGyroscopeBias(map<double, ImageFrame> &all_image_frame,
  ...
  spdlog::warn("gyroscope bias initial calibration {}",
               fmt::streamed(delta_bg.transpose()));
...
bool LinearAlignment(map<double, ImageFrame> &all_image_frame, Vector3d &g,
  ...
  spdlog::debug("estimated scale: {}", s);
  g = x.segment<3>(n_state - 4);
  spdlog::debug("result g {0} {1}", fmt::streamed(g.norm()),
                fmt::streamed(g.transpose()));
  ...
  s = (x.tail<1>())(0) / 100.0;
  (x.tail<1>())(0) = s;
  spdlog::debug("refine g {0} {1}", fmt::streamed(g.norm()),
                fmt::streamed(g.transpose()));
  if (s < 0.0)
    return false;
  else
