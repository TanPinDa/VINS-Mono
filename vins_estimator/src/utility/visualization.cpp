...
EstimatorPublisher::EstimatorPublisher(ros::NodeHandle &n)
    : cameraposevisual(0, 1, 0, 1), keyframebasevisual(0.0, 0.0, 1.0, 1.0) {
...
void EstimatorPublisher::pubLatestOdometry(const Eigen::Vector3d &P,
...
void EstimatorPublisher::printStatistics(const Estimator &estimator, double t) {
  ...
  // printf("position: %f, %f, %f\r", estimator.position[WINDOW_SIZE].x(),
  // estimator.position[WINDOW_SIZE].y(), estimator.position[WINDOW_SIZE].z());
  ROS_DEBUG_STREAM("position: " << estimator.position[WINDOW_SIZE].transpose());
  ROS_DEBUG_STREAM(
      "orientation: " << estimator.linear_velocity[WINDOW_SIZE].transpose());
  ...
  sum_of_path += (estimator.position[WINDOW_SIZE] - last_path).norm();
  last_path = estimator.position[WINDOW_SIZE];
  ROS_DEBUG("sum of path %f", sum_of_path);
  if (ESTIMATE_TD) ROS_INFO("td %f", estimator.td);
}

void EstimatorPublisher::pubOdometry(const Estimator &estimator,
                                     const std_msgs::Header &header) {
  if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR) {
    ...
    odometry.twist.twist.linear.x = estimator.linear_velocity[WINDOW_SIZE].x();
    odometry.twist.twist.linear.y = estimator.linear_velocity[WINDOW_SIZE].y();
    odometry.twist.twist.linear.z = estimator.linear_velocity[WINDOW_SIZE].z();
    ...
    correct_t = estimator.drift_correct_r * estimator.position[WINDOW_SIZE] +
    ...
    foutC.precision(5);
    foutC << estimator.position[WINDOW_SIZE].x() << ","
          << estimator.position[WINDOW_SIZE].y() << ","
          << estimator.position[WINDOW_SIZE].z() << "," << tmp_Q.w() << ","
          << tmp_Q.x() << "," << tmp_Q.y() << "," << tmp_Q.z() << ","
          << estimator.linear_velocity[WINDOW_SIZE].x() << ","
          << estimator.linear_velocity[WINDOW_SIZE].y() << ","
          << estimator.linear_velocity[WINDOW_SIZE].z() << "," << endl;
    foutC.close();
  }
}

void EstimatorPublisher::pubKeyPoses(const Estimator &estimator,
                                     const std_msgs::Header &header) {
...
void EstimatorPublisher::pubCameraPose(const Estimator &estimator,
                                       const std_msgs::Header &header) {
  ...
  if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR) {
    int i = idx2;
    Vector3d P =
        estimator.position[i] + estimator.orientation[i] * estimator.tic[0];
...
void EstimatorPublisher::pubPointCloud(const Estimator &estimator,
                                       const std_msgs::Header &header) {
  ...
  for (auto &it_per_id : estimator.f_manager.feature) {
    ...
    Vector3d w_pts_i = estimator.orientation[imu_i] *
...
void EstimatorPublisher::pubTF(const Estimator &estimator,
                               const std_msgs::Header &header) {
  if (estimator.solver_flag != Estimator::SolverFlag::NON_LINEAR) return;
...
void EstimatorPublisher::pubKeyframe(const Estimator &estimator) {
  ...
  if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR &&
    ...
    int i = WINDOW_SIZE - 2;
    // Vector3d P = estimator.position[i] + estimator.orientation[i] *
    // estimator.tic[0];
    ...
    // what about hedaer. Perhaps based on the full code we will know the
    // header.
