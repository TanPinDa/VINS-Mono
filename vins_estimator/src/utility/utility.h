...
class Utility {
 public:
   ...
   * @param delta_rotation_vector Input delta rotation vector representing the
   * small rotation.
   * @return Quaternion representing the small rotation.
   */
  template <typename Derived>
  static Eigen::Quaternion<typename Derived::Scalar> deltaQuat(
      const Eigen::MatrixBase<Derived> &delta_rotation_vector) {
    ...
    Eigen::Matrix<Scalar_t, 3, 1> half_delta_rotation =
        delta_rotation_vector / static_cast<Scalar_t>(2.0);
    ;
