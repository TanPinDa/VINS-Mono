...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
	...
	if (loop_index != -1)
	{
        ...
        if (cur_kf->findConnection(old_kf, matched_2d_old_norm, matched_id))
        {
            if (on_keyframe_connection_found_cb_) {
                on_keyframe_connection_found_cb_(cur_kf, old_kf, matched_2d_old_norm, matched_id);
            }
...
void PoseGraph::setOnKeyFrameConnectionFoundCallback(std::function<void(KeyFrame*, KeyFrame*, vector<cv::Point2f>&, vector<double>&)> on_keyframe_connection_found_cb) {
