...
void on_keyframe_loaded_callback(KeyFrame* keyframe,
                                 int pose_graph_load_count) {
  Vector3d P;
  Matrix3d R;
...
void on_optimization_step_completed_callback(
...
void on_keyframe_connection_found_callback(
    KeyFrame* cur_kf, KeyFrame* old_kf,
    vector<cv::Point2f>& matched_2d_old_norm, vector<double>& matched_id) {
  {
    sensor_msgs::ImagePtr msg =
        cv_bridge::CvImage(std_msgs::Header(), "bgr8", cur_kf->getThumbImage())
            .toImageMsg();
...
int main(int argc, char** argv) {
  ...
  posegraph.setOnOptimizationStepCompletedCallback(
      on_optimization_step_completed_callback);

  // OnKeyFrameAdded
  posegraph.setOnNewEdgeCallback(on_new_edge_callback);
  posegraph.setOnNewLoopEdgeCallback(on_new_loopedge_callback);
  posegraph.setOnKeyFrameConnectionFoundCallback(
      on_keyframe_connection_found_callback);
